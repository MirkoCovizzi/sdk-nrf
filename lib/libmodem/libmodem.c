...
#error  libmodem must be run as non-secure firmware.\
...
static int _libmodem_init(const struct device *unused)
{
	if (!first_time_init) {
	...
	if (IS_ENABLED(CONFIG_LIBMODEM_SYS_INIT)) {
...
SYS_INIT(_libmodem_init, POST_KERNEL, 0);
#endif
