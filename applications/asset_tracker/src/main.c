...
static K_SEM_DEFINE(libmodem_initialized, 0, 1);
...
int lwm2m_carrier_event_handler(const lwm2m_carrier_event_t *event)
{
	...
	case LWM2M_CARRIER_EVENT_BSDLIB_INIT:
		...
		k_sem_give(&libmodem_initialized);
		break;
...
void handle_libmodem_init_ret(void)
{
#if defined(CONFIG_LIBMODEM)
	int ret = libmodem_get_init_ret();
...
#if defined(CONFIG_LWM2M_CARRIER)
	k_sem_take(&libmodem_initialized, K_FOREVER);
#else
	handle_libmodem_init_ret();
#endif
